...
def sample_with_zeros() -> pd.DataFrame:
    ...
    По Section 6: строки с amount == 0 исключаются из MRR.
    """
    rows = [
...
def sample_new_customers() -> pd.DataFrame:
    ...
    cust_004 появляется впервые в декабре — новый клиент (Section 5).
    """
    rows = [
        # ноябрь — существующие клиенты
        {"customer_id": "cust_001", "date": "2024-11-01", "amount": 100.0, "status": "active", "currency": "USD"},
        {"customer_id": "cust_002", "date": "2024-11-01", "amount": 100.0, "status": "active", "currency": "USD"},
        ...
        {"customer_id": "cust_004", "date": "2024-12-01", "amount": 150.0, "status": "active", "currency": "USD"},
...
def sample_reactivation() -> pd.DataFrame:
    ...
    """
    rows = []
    # Базовые клиенты (cust_001..005) — все месяцы
    for m in range(1, 13):
    ...
    # cust_react: активен январь–август, отсутствует сентябрь–ноябрь, возвращается в декабре
...
def sample_annual() -> pd.DataFrame:
    ...
    cust_annual: большие суммы — исключается из реактивации при любой длительности отсутствия.
    """
    rows = []
    # 5 обычных клиентов ARPU=100, все месяцы
    for m in range(1, 13):
        month_str = f"2024-{m:02d}-01"
        for cid in range(1, 6):
            rows.append({
    ...
    # cust_annual: отсутствовал 3 мес, max(amount) = 800 > 6 × 100
    for m in [1, 2, 3, 4, 5, 6, 7, 8]:
        rows.append({
...
def sample_churn_refund() -> pd.DataFrame:
    ...
    клиент churned И имеет refund в том же месяце.
    Соответствует tests/fixtures/sample_churn_refund.csv (Section 17).
    """
    rows = [
        # Ноябрь: cust_001 и cust_002 активны
        {"customer_id": "cust_001", "date": "2024-11-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
        {"customer_id": "cust_002", "date": "2024-11-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
        {"customer_id": "cust_003", "date": "2024-11-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
        {"customer_id": "cust_004", "date": "2024-11-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
        {"customer_id": "cust_005", "date": "2024-11-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
        # Декабрь: cust_001 churned + refund (Scenario C)
...
def sample_cross_period() -> pd.DataFrame:
    ...
    rows = [
        ...
        {"customer_id": "cust_004", "date": "2024-11-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
        {"customer_id": "cust_005", "date": "2024-11-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
        # Декабрь: cust_001 churned (Scenario D — prior refund не учитывается)
        {"customer_id": "cust_002", "date": "2024-12-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
        {"customer_id": "cust_003", "date": "2024-12-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
        {"customer_id": "cust_004", "date": "2024-12-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
        {"customer_id": "cust_005", "date": "2024-12-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
...
def sample_cohort_zero_amount() -> pd.DataFrame:
    ...
    ИСПРАВЛЕНИЕ: предыдущая версия создавала всех клиентов из одной когорты
    (январь), что давало < 3 когорт → None. Теперь клиенты из 3 разных когорт.
    Структура:
...
def sample_nrr_high() -> pd.DataFrame:
    ...
    Достигается резким ростом MRR при минимальном предыдущем MRR.
    """
    rows = [
        # prev_month: 1 клиент, 100
        {"customer_id": "cust_001", "date": "2024-11-01", "amount": 100.0, "status": "active", "currency": "USD"},
        # last_month: тот же клиент + много новых  → NRR >> 200%
        {"customer_id": "cust_001", "date": "2024-12-01", "amount": 100.0, "status": "active", "currency": "USD"},
...
class TestARPU:
    ...
    def test_arpu_zero_active(self):
        """
        Если нет активных клиентов — вернуть 0.0, не делить на 0 (Section 6).
        """
        rows = [
...
class TestTotalRevenue:
    ...
    def test_total_revenue_excludes_negatives(self, sample_with_negatives):
        ...
        # Положительные: cust_001 нояб(100)+дек(100), cust_002 нояб(50), cust_003 нояб(200)+дек(200)
        assert total > 0
        # Убеждаемся, что -50 не добавляется
        all_positive = sum(
            r["amount"]
...
class TestNewMRR:
    ...
    def test_new_mrr_excludes_reactivated(self, sample_reactivation):
        ...
        # cust_react — реактивация, не новый
        assert react_mrr > 0
        # new_mrr не должен содержать сумму cust_react
...
class TestReactivationMRR:
    ...
    def test_reactivation_mrr_excludes_annual(self, sample_annual):
        ...
        # cust_annual: max(800) > 6 × ARPU → не реактивация
...
class TestGrowthRate:
    ...
    def test_growth_rate_none_prev_mrr_zero(self):
        ...
        """
        rows = [
            # Ноябрь: все с amount=0 → MRR_prev = 0
            *[{"customer_id": f"c{i}", "date": "2024-11-01",
...
class TestChurnRate:
    ...
    def test_churn_rate_none_active_prev_zero(self):
        ...
        """
        rows = [
            # Ноябрь: только churned строки (не active)
            *[{"customer_id": f"c{i}", "date": "2024-11-01",
...
class TestRevenueChurn:
    ...
    def test_revenue_churn_scenario_a(self):
        ...
        rows = [
            ...
            {"customer_id": "cust_004", "date": "2024-11-01", "amount": 100.0, "status": "active",  "currency": "USD"},
            {"customer_id": "cust_005", "date": "2024-11-01", "amount": 100.0, "status": "active",  "currency": "USD"},
            # Декабрь: cust_001 ушёл (Scenario A)
            {"customer_id": "cust_002", "date": "2024-12-01", "amount": 100.0, "status": "active",  "currency": "USD"},
            {"customer_id": "cust_003", "date": "2024-12-01", "amount": 100.0, "status": "active",  "currency": "USD"},
            {"customer_id": "cust_004", "date": "2024-12-01", "amount": 100.0, "status": "active",  "currency": "USD"},
            {"customer_id": "cust_005", "date": "2024-12-01", "amount": 100.0, "status": "active",  "currency": "USD"},
    ...
    def test_revenue_churn_scenario_b(self):
        ...
        rows = [
            ...
            {"customer_id": "cust_004", "date": "2024-11-01", "amount": 100.0, "status": "active",  "currency": "USD"},
            {"customer_id": "cust_005", "date": "2024-11-01", "amount": 100.0, "status": "active",  "currency": "USD"},
            # Декабрь: cust_002 имеет рефанд, но не churned (Scenario B)
            {"customer_id": "cust_001", "date": "2024-12-01", "amount": 100.0,  "status": "active",  "currency": "USD"},
    ...
    def test_revenue_churn_scenario_c(self, sample_churn_refund):
        ...
        # cust_001: churned + refund → только prev amount = 100, НЕ 200
        assert rev_churn == pytest.approx(100.0)
    ...
    def test_revenue_churn_scenario_d(self, sample_cross_period):
        ...
        (Section 8). Prior-period refund исключается.
        """
...
class TestNRR:
    ...
    def test_nrr_basic(self, sample_basic):
        """NRR вычисляется и попадает в диапазон [0, 999] (Section 6)."""
...
class TestCohortTable:
    ...
    def test_cohort_max_12_cohorts(self):
        ...
        rows = []
        for m in range(1, 16):  # 15 когорт
            month_str = f"2023-{m:02d}-01" if m <= 12 else f"2024-{m-12:02d}-01"
            for cid in range(1, 6):
                rows.append({
                    "customer_id": f"m{m}_c{cid}",
                    "date": month_str,
                    "amount": 100.0,
                    "status": "active",
                    "currency": "USD",
...
class TestGetDataQualityFlags:
    ...
    def test_last_month_fallback_triggered(self):
        ...
        """
        rows = [
            # Только 3 клиента в последнем месяце < 5 уникальных
            *[{"customer_id": f"c{i}", "date": "2024-12-01",
               "amount": 100.0, "status": "active", "currency": "USD"} for i in range(1, 4)],
        ]
        df = _make_df(rows)

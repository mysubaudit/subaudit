...
Согласно Section 16 (Development Order), Step 6:
    auth/supabase_auth.py + payments/gumroad.py + auth_callback.py
...
""", unsafe_allow_html=True)
    ...
    # После изменения Email Template токен приходит напрямую как "token"
    # -------------------------------------------------------------------
    query_params = st.query_params

    # Supabase magic link передаёт токен как "token" в query params
    token: str | None = (
        query_params.get("token")
        or query_params.get("token_hash")
    ...
    # -------------------------------------------------------------------
    # Checkpoint 1 (Section 13) — spinner уже внутри get_subscription_status() в gumroad.py
    ...
    # и subscription_warning_reason в session_state внутри gumroad.py.
    ...
    if st.session_state.get("subscription_warning"):
        ...
        # Логика определения post-upgrade находится в gumroad.py;

...
**Спецификация:** `SubAudit_Spec.docx` в корне проекта.
...
## Текущие цены (обновлено 2026-05-16)
...
**Важно:** При изменении цен в Gumroad админке убедиться, что Product IDs остаются теми же (`starter`, `pro`).
...
1. **Спецификация — главный документ.** Все решения строго по ней.
...
1. Прочитать спецификацию `SubAudit_Spec.docx` (если доступна)
...
*Спецификация (`SubAudit_Spec.docx`) остаётся главным документом.*

...
|---|--------|--------|-----------|
| v3.2.1 | Каталог сигнатур | 🔜 **СЛЕДУЮЩИЙ** | `_PRESET_SIGNATURES` в `app/core/presets.py` для 6 источников |
| v3.2.2 | Детектор формата | ⏳ | `detect_preset(df, columns)` — сравнение колонок с сигнатурами |
| v3.2.3 | Интеграция в upload flow | ⏳ | Вызов detect_preset() после парсинга CSV, сохранение в session_state |
| v3.2.4 | UI на mapping-странице | ⏳ | Зелёный badge с именем пресета, кнопка сброса на manual |
| v3.2.5 | Авто-скип mapping | ⏳ | Пропуск mapping при распознанном формате (флажок в настройках) |
...
"**v3.2.1 — Каталог сигнатур колонок** (SPEC.md §8 v3.2.1)
